...
const favorites = async (limit, context) => {
    let section = [];
    ...
    const favorites = await query.findAllLimitByConditions(conditions, Favorite, select, join, order, 1, limit);
    ...
    const sectionFavorites = {
        group_type: 3,
        group_name: "Favorite List",
        section: 13,
        loadmore: `${baseUrl}/loadmore?query=query{loadmoreItems(input:{section:$section,page:$page,limit:$limit}){status{code message}result{items{movie_type movie_uuid movie_name poster_v poster_h}}}}`,
        items: itemsFavorite
    };
    section.push(sectionFavorites);
